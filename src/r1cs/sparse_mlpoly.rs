...
pub struct SparseMatPolynomial<F: PrimeField, G: CurveGroup> {
  num_vars_x: usize,
  num_vars_y: usize,
...
impl<F: PrimeField> Derefs<F> {
  pub fn new(row_ops_val: Vec<DensePolynomial<F>>, col_ops_val: Vec<DensePolynomial<F>>) -> Self {
    ...
    let derefs = {
      ...
      let comb = DensePolynomial::merge_r1cs(row_ops_val.iter().chain(col_ops_val.iter()));
  ...
  pub fn commit<G>(&self, gens: &PolyCommitmentGens<G>) -> DerefsCommitment<G>
  where
...
#[derive(Debug, CanonicalSerialize, CanonicalDeserialize)]
pub struct DerefsEvalProof<F: PrimeField, G: CurveGroup> {
  proof_derefs: PolyEvalProof<G>,
  _marker: PhantomData<F>,
}

impl<F: PrimeField, G: CurveGroup> DerefsEvalProof<F, G>
  where
    G: CurveGroup<ScalarField = F>,
  {
    ...
    fn prove_single(
      ...
      let (r_joint, eval_joint) = {
        let challenges =
          <Transcript as ProofTranscript<G>>::challenge_vector(transcript, b"challenge_combine_n_to_one", evals.len().log_2() as usize);
    ...
    pub fn prove(
      ...
      <Transcript as ProofTranscript<G>>::append_protocol_name(transcript, DerefsEvalProof::<F, G>::protocol_name());
      ...
      DerefsEvalProof { proof_derefs, _marker: PhantomData }
    }
    ...
    fn verify_single(
      ...
      // decommit the joint polynomial at r_joint
      <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"joint_claim_eval", &joint_claim_eval);
...
impl<G: CurveGroup> AppendToTranscript<G> for DerefsCommitment<G> {
  fn append_to_transcript<T: ProofTranscript<G>>(&self, label: &'static [u8], transcript: &mut T) {
    <T as ProofTranscript<G>>::append_message(transcript, b"derefs_commitment", b"begin_derefs_commitment");
    self.comm_ops_val.append_to_transcript(label, transcript);
    <T as ProofTranscript<G>>::append_message(transcript, b"derefs_commitment", b"end_derefs_commitment");
  }
}
...
pub struct SparseMatPolyCommitmentGens<G: CurveGroup> {
...
impl<G: CurveGroup> AppendToTranscript<G> for SparseMatPolyCommitment<G> {
  fn append_to_transcript<T: ProofTranscript<G>>(&self, _label: &'static [u8], transcript: &mut T) {
    <T as ProofTranscript<G>>::append_u64(transcript, b"batch_size", self.batch_size as u64);
    <T as ProofTranscript<G>>::append_u64(transcript, b"num_ops", self.num_ops as u64);
    <T as ProofTranscript<G>>::append_u64(transcript, b"num_mem_cells", self.num_mem_cells as u64);
    self
      .comm_comb_ops
...
impl<F: PrimeField, G: CurveGroup> SparseMatPolynomial<F, G> {
  ...
  fn multi_sparse_to_dense_rep(
    sparse_polys: &[&SparseMatPolynomial<F, G>],
    ...
    let comb_ops = DensePolynomial::merge_r1cs(
      row
        .ops_addr
        .iter()
  ...
  ) -> (SparseMatPolyCommitment<G>, MultiSparseMatPolynomialAsDense<F>)
  where
    G: CurveGroup<ScalarField = F>
  {
...
impl<F: PrimeField> Layers<F> {
  fn build_hash_layer(
    ...
    let hash_func = |addr: &F, val: &F, ts: &F| -> F {
...
#[derive(Debug, CanonicalSerialize, CanonicalDeserialize)]
struct HashLayerProof<F: PrimeField, G: CurveGroup> {
  eval_row: (Vec<F>, Vec<F>, F),
  eval_col: (Vec<F>, Vec<F>, F),
  eval_val: Vec<F>,
  eval_derefs: (Vec<F>, Vec<F>),
  proof_ops: PolyEvalProof<G>,
  proof_mem: PolyEvalProof<G>,
  proof_derefs: DerefsEvalProof<F, G>,
}

impl<F: PrimeField, G: CurveGroup> HashLayerProof<F, G> {
  fn protocol_name() -> &'static [u8] {
  ...
  where
    G: CurveGroup<ScalarField = F>
  {
    <Transcript as ProofTranscript<G>>::append_protocol_name(transcript, HashLayerProof::<F, G>::protocol_name());
    ...
    evals_ops.resize(evals_ops.len().next_power_of_two(), F::zero());
    <Transcript as ProofTranscript<G>>::append_scalars(transcript, b"claim_evals_ops", &evals_ops);
    let challenges_ops =
    <Transcript as ProofTranscript<G>>::challenge_vector(transcript, b"challenge_combine_n_to_one", evals_ops.len().log_2() as usize);
    ...
    <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"joint_claim_eval_ops", &joint_claim_eval_ops);
    ...
    let challenges_mem =
    <Transcript as ProofTranscript<G>>::challenge_vector(transcript, b"challenge_combine_two_to_one", evals_mem.len().log_2() as usize);
    ...
    <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"joint_claim_eval_mem", &joint_claim_eval_mem);
  ...
  fn verify(
    &self,
    rand: (&Vec<F>, &Vec<F>),
    claims_row: &(F, Vec<F>, Vec<F>, F),
    claims_col: &(F, Vec<F>, Vec<F>, F),
    claims_dotp: &[F],
    ...
    G: CurveGroup<ScalarField = F>
  {
    ...
    evals_ops.extend(eval_val_vec);
    evals_ops.resize(evals_ops.len().next_power_of_two(), F::zero());
    <Transcript as ProofTranscript<G>>::append_scalars(transcript, b"claim_evals_op", &evals_ops);
    let challenges_ops =
    <Transcript as ProofTranscript<G>>::challenge_vector(transcript, b"challenge_combine_n_to_one", evals_ops.len().log_2() as usize);
    ...
    HashLayerProof::<F, G>::verify_helper(
      &(rand_mem, rand_ops),
      claims_row,
      eval_row_ops_val,
      eval_ops_addr,
...
#[derive(Debug, CanonicalSerialize, CanonicalDeserialize)]
struct ProductLayerProof<F: PrimeField, G: CurveGroup> {
  eval_row: (F, Vec<F>, Vec<F>, F),
  eval_col: (F, Vec<F>, Vec<F>, F),
  eval_val: (Vec<F>, Vec<F>),
  proof_mem: ProductCircuitEvalProofBatched<F, G>,
  proof_ops: ProductCircuitEvalProofBatched<F, G>,
}

impl<F: PrimeField, G: CurveGroup> ProductLayerProof<F, G> {
  fn protocol_name() -> &'static [u8] {
  ...
  where
    G: CurveGroup<ScalarField = F>,
  {
    <Transcript as ProofTranscript<G>>::append_protocol_name(transcript, ProductLayerProof::<F, G>::protocol_name());
    ...
    <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"claim_row_eval_init", &row_eval_init);
    <Transcript as ProofTranscript<G>>::append_scalars(transcript, b"claim_row_eval_read", &row_eval_read);
    <Transcript as ProofTranscript<G>>::append_scalars(transcript, b"claim_row_eval_write", &row_eval_write);
    <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"claim_row_eval_audit", &row_eval_audit);
    ...
    <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"claim_col_eval_init", &col_eval_init);
    <Transcript as ProofTranscript<G>>::append_scalars(transcript, b"claim_col_eval_read", &col_eval_read);
    <Transcript as ProofTranscript<G>>::append_scalars(transcript, b"claim_col_eval_write", &col_eval_write);
    <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"claim_col_eval_audit", &col_eval_audit);
    ...
    for i in 0..derefs.row_ops_val.len() {
      ...
      <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"claim_eval_dotp_left", &eval_dotp_left);
      <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"claim_eval_dotp_right", &eval_dotp_right);
    ...
    product_layer_proof.serialize_compressed(&mut product_layer_proof_encoded).unwrap();
    let msg = format!(
    ...
    G: CurveGroup<ScalarField = F>,
  {
    ...
    for i in 0..num_instances {
      ...
      <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"claim_eval_dotp_left", &eval_dotp_left[i]);
      <Transcript as ProofTranscript<G>>::append_scalar(transcript, b"claim_eval_dotp_right", &eval_dotp_right[i]);
...
impl SparseMatPolyEvalProof {
  ...
  fn equalize(rx: &[F], ry: &[F]) -> (Vec<F>, Vec<F>) {
    match rx.len().cmp(&ry.len()) {
      Ordering::Less => {
        let diff = ry.len() - rx.len();
        let mut rx_ext = vec![F::zero(); diff];
        rx_ext.extend(rx);
        (rx_ext, ry.to_vec())
      }
      Ordering::Greater => {
        let diff = rx.len() - ry.len();
        let mut ry_ext = vec![F::zero(); diff];
        ry_ext.extend(ry);
        (rx.to_vec(), ry_ext)
      }
